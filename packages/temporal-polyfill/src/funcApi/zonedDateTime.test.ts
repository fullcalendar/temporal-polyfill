...
function expectRoundToYearEquals(isoString: string, expected: string) {
  expectZonedDateTimeEquals(
    ZonedDateTimeFns.roundToYear(
      ZonedDateTimeFns.fromString(isoString, CalendarFns.getBasic),
    ),
    ZonedDateTimeFns.fromString(expected, CalendarFns.getBasic),
  )
}
...
describe('roundToDay', () => {
  ...
  it('rounds to each named day-or-time unit', () => {
    ...
    for (const [floorExpected, halfExpandExpected, roundTo] of cases) {
      expectZonedDateTimeEquals(
        roundTo(zdt),
        ZonedDateTimeFns.fromString(halfExpandExpected, CalendarFns.getBasic),
      )
      expectZonedDateTimeEquals(
        roundTo(zdt, 'floor'),
        ZonedDateTimeFns.fromString(floorExpected, CalendarFns.getBasic),
      )
      expectZonedDateTimeEquals(
...
describe('toString', () => {
  ...
  it('has a simple no-options variant', () => {
    ...
    expect(ZonedDateTimeFns.toBasicString(zdt)).toBe(
...
describe('withWeekOfYear', () => {
  it('works with ISO calendar (and coerces to integer)', () => {
    const zdt0 = ZonedDateTimeFns.fromString(
      '2024-02-27T12:30:00[America/New_York]',
      CalendarFns.getBasic, // weekOfYear:9, yearOfWeek:2024
    )
    const zdtExp = ZonedDateTimeFns.fromString(
  ...
  it('works without options', () => {
    ...
    expectZonedDateTimeEquals(
      ...
      ZonedDateTimeFns.fromString(
        ...
        CalendarFns.getBasic, // next Monday
      ),
    )
  })
  ...
  it('works with roundingMode option', () => {
    ...
    expectZonedDateTimeEquals(
      ...
      ZonedDateTimeFns.fromString(
        ...
        CalendarFns.getBasic, // this Monday
      ),
    )
  })
...
describe('startOfWeek', () => {
  ...
  it('uses the first real instant when the week starts after a skipped midnight', () => {
    const zdt = ZonedDateTimeFns.fromString(
      ...
      CalendarFns.getBasic, // Wednesday
    )
...
describe('diffWeeks', () => {
  it('gives exact result when no options/roundingMode specified', () => {
    ...
    const zdt1 = ZonedDateTimeFns.fromString(
      ...
      CalendarFns.getBasic, // -04:00 (not affected!)
    )
...
describe('diffHours', () => {
  it('gives exact result when no options/roundingMode specified', () => {
    ...
    const zdt1 = ZonedDateTimeFns.fromString(
      ...
      CalendarFns.getBasic, // -04:00 (looses one hour)
    )
