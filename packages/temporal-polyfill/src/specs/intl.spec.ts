...
describe('Intl', () => {
    ...
    for (const id of calendars) {
      ...
      itOrSkip(id)(`${id} months check for year ${year}`, () => {
        ...
        for (
          let i = monthsInYear,
            leapMonthIndex,
            monthStart: Temporal.PlainDate | undefined;
          i >= 1;
          i--
        ) {
  ...
  describe('Japanese eras', () => {
    ...
    it('Heisei (1989-2019)', () => {
      const date = Temporal.PlainDate.from({ era: 'heisei', eraYear: 2, month: 1, day: 1, calendar: 'japanese' })
      equal(`${date}`, '1990-01-01[u-ca=japanese]')
    })
    it('Showa (1926-1989)', () => {
      const date = Temporal.PlainDate.from({ era: 'showa', eraYear: 2, month: 1, day: 1, calendar: 'japanese' })
      equal(`${date}`, '1927-01-01[u-ca=japanese]')
    })
    it('Taisho (1912-1926)', () => {
      const date = Temporal.PlainDate.from({ era: 'taisho', eraYear: 2, month: 1, day: 1, calendar: 'japanese' })
      equal(`${date}`, '1913-01-01[u-ca=japanese]')
    })
    it('Meiji (1868-1912)', () => {
      const date = Temporal.PlainDate.from({ era: 'meiji', eraYear: 2, month: 1, day: 1, calendar: 'japanese' })
    ...
    it("`with` doesn't crash when constraining dates out of bounds of the current era", () => {
      ...
      const date = Temporal.PlainDate.from('1989-01-07')
