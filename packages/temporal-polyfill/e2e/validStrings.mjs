...
bad.forEach((name) => {
  return timezoneNames.delete(name)
})
...
function toProduction(productionLike) {
  if (typeof productionLike === 'string') {
    return new Literal(productionLike)
  }

  if (Array.isArray(productionLike)) {
    if (productionLike.length === 1) {
      return new Optional(productionLike[0])
    }
...
class Choice {
  constructor(...options) {
    this.options = options.reduce((acc, production) => {
      ...
      if (production instanceof Choice) {
        return acc.concat(production.options)
      }
...
function zeroPaddedInclusive(start, end, len) {
  const range = new Array(end - start + 1).fill().map((_, ix) => {
    return ix + start
  })
  const padded = range.map((num) => {
    return new Literal(`${num}`.padStart(len, '0'))
  })
...
class Sequence {
  constructor(...productions) {
    this.productions = productions.reduce((acc, production) => {
      ...
      if (production instanceof Sequence) {
        return acc.concat(production.productions)
      }
      return acc.concat([production])
    }, [])
  }
...
const dateYear = withCode(
  ...
  (data, result) => {
    return (data.year = +result.replace('\u2212', '-'))
  }
)
const dateMonth = withCode(zeroPaddedInclusive(1, 12, 2), (data, result) => {
  return (data.month = +result)
})
const dateDay = withCode(zeroPaddedInclusive(1, 31, 2), (data, result) => {
  return (data.day = +result)
})

const timeHour = withCode(hour, (data, result) => {
  return (data.hour = +result)
})
const timeMinute = withCode(minuteSecond, (data, result) => {
  return (data.minute = +result)
})
...
const timeZoneUTCOffsetSign = withCode(sign, (data, result) => {
  return (data.offsetSign = result === '-' || result === '\u2212' ? '-' : '+')
})
const timeZoneUTCOffsetHour = withCode(hour, (data, result) => {
  return (data.offsetHour = +result)
})
const timeZoneUTCOffsetMinute = withCode(minuteSecond, (data, result) => {
  return (data.offsetMinute = +result)
})
const timeZoneUTCOffsetSecond = withCode(minuteSecond, (data, result) => {
  return (data.offsetSecond = +result)
})
  ...
  seq(
    ...
    choice(
      [
        ...
        [timeZoneUTCOffsetSecond, [timeZoneUTCOffsetFraction]],
      ],
      seq(':', timeZoneUTCOffsetMinute, [
        ':',
...
const timeZoneUTCOffsetName = seq(
  ...
  hour,
  choice(
    [minuteSecond, [minuteSecond, [fraction]]],
...
const timeZoneBracketedName = withCode(
  ...
  (data, result) => {
    return (data.ianaName =
      ES.GetCanonicalTimeZoneIdentifier(result).toString())
  }
)
const timeZoneBracketedAnnotation = seq('[', timeZoneBracketedName, ']')
const timeZoneIANAName = withCode(choice(...timezoneNames), (data, result) => {
  return (data.ianaName = ES.GetCanonicalTimeZoneIdentifier(result).toString())
})
const timeZone = withCode(
  choice(
    timeZoneUTCOffset,
    seq([timeZoneNumericUTCOffset], timeZoneBracketedAnnotation)
  ),
  (data) => {
    if (!('offset' in data)) {
...
const temporalTimeZoneIdentifier = withCode(
  choice(timeZoneNumericUTCOffset, timeZoneIANAName),
  (data) => {
    if (!('offset' in data)) {
      data.offset = undefined
    }
...
const calendarName = withCode(choice(...calendarNames), (data, result) => {
  return (data.calendar = result)
})
const calendar = seq('[u-ca=', calendarName, ']')
const timeSpec = seq(
  timeHour,
  choice(
    [':', timeMinute, [':', timeSecond, [timeFraction]]],
    seq(timeMinute, [timeSecond, [timeFraction]])
  )
)
...
const date = choice(
  seq(dateYear, '-', dateMonth, '-', dateDay),
  seq(dateYear, dateMonth, dateDay)
)
...
const durationFractionalPart = withCode(
  between(1, 9, digit()),
  (data, result) => {
    const fraction = result.padEnd(9, '0')
...
const durationSeconds = seq(
  withCode(oneOrMore(digit()), (data, result) => {
    return (data.seconds = +result * data.factor)
  }),
  [durationFraction],
  secondsDesignator
)
const durationMinutes = seq(
  withCode(oneOrMore(digit()), (data, result) => {
    return (data.minutes = +result * data.factor)
  }),
  minutesDesignator,
  [durationSeconds]
)
const durationHours = seq(
  withCode(oneOrMore(digit()), (data, result) => {
    return (data.hours = +result * data.factor)
  }),
  hoursDesignator,
  [durationMinutes]
)
const durationTime = seq(
  durationTimeDesignator,
  choice(durationHours, durationMinutes, durationSeconds)
)
const durationDays = seq(
  withCode(oneOrMore(digit()), (data, result) => {
    return (data.days = +result * data.factor)
  }),
  daysDesignator
)
const durationWeeks = seq(
  withCode(oneOrMore(digit()), (data, result) => {
    return (data.weeks = +result * data.factor)
  }),
  weeksDesignator,
  [durationDays]
)
const durationMonths = seq(
  withCode(oneOrMore(digit()), (data, result) => {
    return (data.months = +result * data.factor)
  }),
  monthsDesignator,
  [durationWeeks]
)
const durationYears = seq(
  withCode(oneOrMore(digit()), (data, result) => {
    return (data.years = +result * data.factor)
  }),
  yearsDesignator,
  [durationMonths]
)
const durationDate = seq(
  choice(durationYears, durationMonths, durationWeeks, durationDays),
  [durationTime]
)
const duration = seq(
  withCode([sign], (data, result) => {
    return (data.factor = result === '-' || result === '\u2212' ? -1 : 1)
  }),
  durationDesignator,
...
for (let count = 0; count < 1000; count++) {
  ...
  try {
    ...
    for (const prop of comparisonItems[mode]) {
