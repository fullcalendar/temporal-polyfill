...
describe('Date', () => {
  ...
  describe('order of operations in until - TODO: add since', () => {
    ...
    for (const [end, start, expected] of cases) {
      it(`${start} until ${end} => ${expected}`, () => {
        const result = PlainDate.from(start).until(end, {
          largestUnit: 'months',
        })
  ...
  describe('date.add() works', () => {
    ...
    it('invalid overflow', () => {
      ;['', 'CONSTRAIN', 'balance', 3, null].forEach((overflow) => {
        return throws(() => {
          return date.add({ months: 1 }, { overflow })
        }, RangeError)
      })
    })
    it('mixed positive and negative values always throw', () => {
      ;['constrain', 'reject'].forEach((overflow) => {
        return throws(() => {
          return date.add({ months: 1, days: -30 }, { overflow })
        }, RangeError)
      })
    })
    it('options may only be an object or undefined', () => {
      ;[{}, () => {}, undefined].forEach((options) => {
        return equal(`${date.add({ months: 1 }, options)}`, '1976-12-18')
      })
    })
  ...
  describe('date.subtract() works', () => {
    ...
    it('Date.from("2019-02-28").subtract({ months: 1 })', () => {
      equal(
        `${PlainDate.from('2019-02-28').subtract({ months: 1 })}`,
        '2019-01-28'
      )
    })
    ...
    it('symmetrical with regard to negative durations', () => {
      equal(
        `${PlainDate.from('1976-11-18').subtract({ years: -43 })}`,
        '2019-11-18'
      )
      equal(
        `${PlainDate.from('2018-12-18').subtract({ months: -11 })}`,
        '2019-11-18'
      )
      equal(
        `${PlainDate.from('2019-10-29').subtract({ days: -20 })}`,
        '2019-11-18'
      )
      equal(
        `${PlainDate.from('2019-01-28').subtract({ months: -1 })}`,
        '2019-02-28'
      )
    })
    it("ignores lower units that don't balance up to a day", () => {
    ...
    it('subtracts lower units that balance up to a day or more', () => {
      ...
      equal(
        `${date.subtract({ nanoseconds: 86400_000_000_000 })}`,
        '2019-11-17'
      )
    })
    it('invalid overflow', () => {
      ;['', 'CONSTRAIN', 'balance', 3, null].forEach((overflow) => {
        return throws(() => {
          return date.subtract({ months: 1 }, { overflow })
        }, RangeError)
      })
    })
    it('mixed positive and negative values always throw', () => {
      ;['constrain', 'reject'].forEach((overflow) => {
        return throws(() => {
          return date.subtract({ months: 1, days: -30 }, { overflow })
        }, RangeError)
      })
    })
    it('options may only be an object or undefined', () => {
      ;[{}, () => {}, undefined].forEach((options) => {
        return equal(`${date.subtract({ months: 1 }, options)}`, '2019-10-18')
      })
    })
    it('object must contain at least one correctly-spelled property', () => {
      throws(() => {
  ...
  describe('date.toString() works', () => {
    ...
    it('shows only non-ISO calendar if calendarName = auto', () => {
      ...
      equal(
        d.withCalendar('gregory').toString({ calendarName: 'auto' }),
    ...
    it('omits non-ISO calendar if calendarName = never', () => {
      equal(
        d.withCalendar('gregory').toString({ calendarName: 'never' }),
        '1976-11-18'
      )
    })
  ...
  describe('Date.from() works', () => {
    ...
    it('Date.from({ year: 1976, month: 11, day: 18 }) == 1976-11-18', () => {
      return equal(
        `${PlainDate.from({ year: 1976, month: 11, day: 18 })}`,
        '1976-11-18'
      )
    })
    ...
    it('can be constructed with monthCode and without month', () => {
      return equal(
        `${PlainDate.from({ year: 1976, monthCode: 'M11', day: 18 })}`,
        '1976-11-18'
      )
    })
    ...
    it('Date.from({ year: 2019, day: 15 }) throws', () => {
      return throws(() => {
        return PlainDate.from({ year: 2019, day: 15 })
      }, TypeError)
    })
    it('Date.from({ month: 12 }) throws', () => {
      return throws(() => {
        return PlainDate.from({ month: 12 })
      }, TypeError)
    })
    it('object must contain at least the required correctly-spelled properties', () => {
      ...
      throws(() => {
        return PlainDate.from({ year: 1976, months: 11, day: 18 })
      }, TypeError)
    })
    it('incorrectly-spelled properties are ignored', () => {
      equal(
        `${PlainDate.from({ year: 1976, month: 11, day: 18, days: 15 })}`,
        '1976-11-18'
      )
    })
    it('Date.from(required prop undefined) throws', () => {
      return throws(() => {
        return PlainDate.from({ year: undefined, month: 11, day: 18 })
      }, TypeError)
    })
    it('Date.from(number) is converted to string', () => {
      return PlainDate.from(19761118).equals(PlainDate.from('19761118'))
    })
    it('basic format', () => {
    ...
    it('no junk at end of string', () => {
      return throws(() => {
        return PlainDate.from('1976-11-18junk')
      }, RangeError)
    })
    it('ignores if a timezone is specified', () => {
      return equal(
        `${PlainDate.from('2020-01-01[Asia/Kolkata]')}`,
        '2020-01-01'
      )
    })
    it('options may only be an object or undefined', () => {
      ;[{}, () => {}, undefined].forEach((options) => {
        return equal(
          `${PlainDate.from({ year: 1976, month: 11, day: 18 }, options)}`,
          '1976-11-18'
        )
      })
    })
    describe('Overflow', () => {
      ...
      it('reject', () => {
        return throws(() => {
          return PlainDate.from(bad, { overflow: 'reject' })
        }, RangeError)
      })
      it('constrain', () => {
      ...
      it('throw when bad overflow', () => {
        ;[
          new PlainDate(1976, 11, 18),
          { year: 2019, month: 1, day: 1 },
          '2019-01-31',
        ].forEach((input) => {
          ;['', 'CONSTRAIN', 'balance', 3, null].forEach((overflow) => {
            return throws(() => {
              return PlainDate.from(input, { overflow })
            }, RangeError)
          })
        })
      })
      it('constrain has no effect on invalid ISO string', () => {
        throws(() => {
          return PlainDate.from('2020-13-34', { overflow: 'constrain' })
        }, RangeError)
      })
    })
  })
  describe('Date.compare works', () => {
    ...
    it('equal', () => {
      return equal(PlainDate.compare(d1, d1), 0)
    })
    it('smaller/larger', () => {
      return equal(PlainDate.compare(d1, d2), -1)
    })
    it('larger/smaller', () => {
      return equal(PlainDate.compare(d2, d1), 1)
    })
    ...
    it('object must contain at least the required properties', () => {
      throws(() => {
        return PlainDate.compare({ year: 1976 }, d2)
      }, TypeError)
      throws(() => {
        return PlainDate.compare(d1, { year: 2019 })
      }, TypeError)
    })
  })
  describe('Date.equal works', () => {
    ...
    const d2 = PlainDate.from('2019-06-30')
    it('equal', () => {
      return assert(d1.equals(d1))
    })
    it('unequal', () => {
      return assert(!d1.equals(d2))
    })
    it('casts argument', () => {
  ...
  describe('Min/max range', () => {
    it('constructing from numbers', () => {
      throws(() => {
        return new PlainDate(-271821, 4, 18)
      }, RangeError)
      throws(() => {
        return new PlainDate(275760, 9, 14)
      }, RangeError)
    ...
    it('constructing from property bag', () => {
      ...
      ;['reject', 'constrain'].forEach((overflow) => {
        ;[tooEarly, tooLate].forEach((props) => {
          throws(() => {
            return PlainDate.from(props, { overflow })
          }, RangeError)
        })
      })
      equal(
        `${PlainDate.from({ year: -271821, month: 4, day: 19 })}`,
        '-271821-04-19'
      )
      equal(
        `${PlainDate.from({ year: 275760, month: 9, day: 13 })}`,
        '+275760-09-13'
      )
    })
    it('constructing from ISO string', () => {
      ;['reject', 'constrain'].forEach((overflow) => {
        ;['-271821-04-18', '+275760-09-14'].forEach((str) => {
          throws(() => {
            return PlainDate.from(str, { overflow })
          }, RangeError)
        })
      })
    ...
    it('converting from YearMonth', () => {
      ...
      throws(() => {
        return min.toPlainDate({ day: 1 })
      }, RangeError)
      throws(() => {
        return max.toPlainDate({ day: 30 })
      }, RangeError)
    ...
    it('converting from MonthDay', () => {
      ...
      throws(() => {
        return jan1.toPlainDate({ year: minYear })
      }, RangeError)
      throws(() => {
        return dec31.toPlainDate({ year: maxYear })
      }, RangeError)
    ...
    it('adding and subtracting beyond limit', () => {
      ...
      ;['reject', 'constrain'].forEach((overflow) => {
        throws(() => {
          return min.subtract({ days: 1 }, { overflow })
        }, RangeError)
        throws(() => {
          return max.add({ days: 1 }, { overflow })
        }, RangeError)
      })
    })
  })
