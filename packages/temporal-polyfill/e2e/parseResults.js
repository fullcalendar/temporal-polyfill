...
function reportStatus(length = testCount) {
  ...
  stdout.write(
    `${length} tests completed in ${secs.toFixed(1)} seconds. (${Math.round(
      length / secs
    )} tests/sec)\n`
  )
}
...
stdin.on('end', function () {
  ...
  const tests = JSON.parse(testOutput).filter((test) => {
    return test.scenario.includes('strict')
  })
  const failedTests = []
  ...
  for (const test of tests) {
    ...
    const message = `${
      PREFIXES[+expectedFailure][+result.pass]
    } ${file} (${scenario})\n`
