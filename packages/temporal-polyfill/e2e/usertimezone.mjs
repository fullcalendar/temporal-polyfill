...
describe.skip('Userland time zone', () => {
  describe('Trivial subclass', () => {
    ...
    it('.id property', () => {
      return equal(obj.id, 'Etc/Custom/UTC_Subclass')
    })
    // FIXME: what should happen in Temporal.TimeZone.from(obj)?
    it('.id is not available in from()', () => {
      throws(() => {
        return Temporal.TimeZone.from('Etc/Custom/UTC_Subclass')
      }, RangeError)
      throws(() => {
        return Temporal.TimeZone.from(
          '2020-05-26T16:02:46.251163036+00:00[Etc/Custom/UTC_Subclass]'
        )
      }, RangeError)
    })
    it('has offset string +00:00', () => {
      return equal(obj.getOffsetStringFor(inst), '+00:00')
    })
    it('converts to DateTime', () => {
      equal(`${obj.getPlainDateTimeFor(inst)}`, '1970-01-01T00:00:00')
      equal(
        `${obj.getPlainDateTimeFor(inst, 'gregory')}`,
    ...
    it('converts to string', () => {
      return equal(`${obj}`, obj.id)
    })
    it('offset prints in instant.toString', () => {
      return equal(
        inst.toString({ timeZone: obj }),
    ...
    it('prints in zdt.toString', () => {
      ...
      equal(
        zdt.toString(),
        '1970-01-01T00:00:00+00:00[Etc/Custom/UTC_Subclass]'
      )
    })
    it('has no next transitions', () => {
      return assert.equal(obj.getNextTransition(), null)
    })
    it('has no previous transitions', () => {
      return assert.equal(obj.getPreviousTransition(), null)
    })
    it('works in Temporal.Now', () => {
      assert(
        Temporal.Now.plainDateTimeISO(obj) instanceof Temporal.PlainDateTime
      )
      assert(
        Temporal.Now.plainDateTime('gregory', obj) instanceof
          Temporal.PlainDateTime
      )
      assert(Temporal.Now.plainDateISO(obj) instanceof Temporal.PlainDate)
      assert(
        Temporal.Now.plainDate('gregory', obj) instanceof Temporal.PlainDate
      )
  ...
  describe('Trivial protocol implementation', () => {
    ...
    it('converts to DateTime', () => {
      equal(
        `${Temporal.TimeZone.prototype.getPlainDateTimeFor.call(obj, inst)}`,
        '1970-01-01T00:00:00'
      )
      equal(
        `${Temporal.TimeZone.prototype.getPlainDateTimeFor.call(
          obj,
          inst,
          'gregory'
        )}`,
    ...
    it('prints in zdt.toString', () => {
      ...
      equal(
        zdt.toString(),
        '1970-01-01T00:00:00+00:00[Etc/Custom/UTC_Protocol]'
      )
    })
    it('works in Temporal.Now', () => {
      assert(
  ...
  describe('sub-minute offset', () => {
    ...
    it('.id property', () => {
      return equal(obj.id, 'Custom/Subminute')
    })
    it('.id is not available in from()', () => {
      throws(() => {
        return Temporal.TimeZone.from('Custom/Subminute')
      }, RangeError)
      throws(() => {
        return Temporal.TimeZone.from(
          '2020-05-26T16:02:46.251163036-00:00:01.111111111[Custom/Subminute]'
        )
      }, RangeError)
    })
    it('has offset string -00:00:01.111111111', () => {
      return equal(obj.getOffsetStringFor(inst), '-00:00:01.111111111')
    })
    it('converts to DateTime', () => {
      equal(`${obj.getPlainDateTimeFor(inst)}`, '1969-12-31T23:59:58.888888889')
      equal(
        `${obj.getPlainDateTimeFor(inst, 'gregory')}`,
        '1969-12-31T23:59:58.888888889[u-ca=gregory]'
      )
    })
    it('converts to Instant', () => {
    ...
    it('offset prints in instant.toString', () => {
      return equal(
        inst.toString({ timeZone: obj }),
        '1969-12-31T23:59:58.888888889-00:00:01.111111111'
      )
    })
    it('prints in zdt.toString', () => {
      const zdt = new Temporal.ZonedDateTime(0n, obj)
      equal(
        zdt.toString(),
        '1969-12-31T23:59:58.888888889-00:00:01.111111111[Custom/Subminute]'
      )
    })
    it('has no next transitions', () => {
      return assert.equal(obj.getNextTransition(), null)
