...
export namespace Temporal {
  ...
  // Use the host's Intl.DurationFormat options when the consumer includes a
  // lib that declares Intl.DurationFormat, without forcing this file to
  // reference es2025.intl for every consumer.
  type DurationFormatOptions =
    typeof Intl extends {
      DurationFormat: {
        new (locales?: unknown, options?: infer Options): unknown;
      };
    }
      ? NonNullable<Options>
      : unknown;
...
export namespace Intl {
  ...
  interface DateTimeFormat extends Omit<globalThis.Intl.DateTimeFormat, 'format' | 'formatToParts' | 'formatRange' | 'formatRangeToParts'> {
    ...
    formatToParts(date?: FormattableTemporalObject | Date | number): globalThis.Intl.DateTimeFormatPart[];
    formatRange(startDate: FormattableTemporalObject | Date | number, endDate: FormattableTemporalObject | Date | number): string;
    formatRangeToParts(startDate: FormattableTemporalObject | Date | number, endDate: FormattableTemporalObject | Date | number): globalThis.Intl.DateTimeRangeFormatPart[];
  }
}
